...
/**
 * VelDataSunkByCalendarYearService 单元测试
 *
 * @author yong.li
 * @since 2026-07-04
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("VelDataSunkByCalendarYearService 单元测试")
class VelDataSunkByCalendarYearServiceTest {

    @InjectMocks
    private VelDataSunkByCalendarYearService service;
    ...
    private static final String REDIS_KEY = "vel:cy:test:key";
    ...
    void testGetVelData_CacheHit_ReturnsCacheData() {
        VelEventData dto = buildDto();
        String cacheJson = "{\"slave_X\":[{\"txnId\":\"t1\",\"ts\":4000,\"v\":100}]}";
        ...
        assertTrue(result.getBeforeData().containsKey("slave_X"));
        assertTrue(result.getAfterData().containsKey("slave_X"));
    }
    ...
    private VelEventData buildDto() {
        ...
        dto.setTxnId("TXN_003");
        dto.setFeatureCode("FEA_003");
        dto.setMasterValue("master_C");
        dto.setSlaveValue("slave_C");
        dto.setTimeMode("cy");
        dto.setTimeWindow(1);
        dto.setTimeUnit("YEAR");
        return dto;
    }
}
