...
/**
 * CalculateMinMethod 单元测试
 *
 * @author yong.li
 * @since 2026-07-04
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("CalculateMinMethod 单元测试")
class CalculateMinMethodTest {

    @InjectMocks
    private CalculateMinMethod calculateMinMethod;
    ...
    void testDoCalculate_NullDto() {
        VelCalculateResult result = calculateMinMethod.doCalculate(null);
    ...
    void testDoCalculate_NullMasterValues() {
        ...
        VelCalculateResult result = calculateMinMethod.doCalculate(dto);
    ...
    @Test
    @DisplayName("doCalculate — 单个主维度，dataMap 有 3 个 key，min 为 3")
    ...
    @Test
    @DisplayName("doCalculate — 单个主维度，dataMap 为 null，min 为 0")
    ...
    @Test
    @DisplayName("doCalculate — 单个主维度，dataMap 为空，min 为 0")
    ...
    @Test
    @DisplayName("doCalculate — 三个主维度 key 数为 2/5/1，min 为 1")
    void testDoCalculate_MultipleMasters_MinOfKeyCounts() {
        VelReadEvent dto = buildBaseDto();
        ...
        // min(2, 5, 1) = 1
        assertEquals(BigDecimal.valueOf(1), result.getResult());
        verify(dataSunkService, times(3)).getVelData(any());
    }

    @Test
    @DisplayName("doCalculate — 三个主维度 key 数全部相同为 2，min 为 2")
    ...
    @Test
    @DisplayName("doCalculate — 四个主维度全部返回 0，min 为 0")
    void testDoCalculate_AllMastersZero() {
        ...
        when(dataSunkService.getVelData(any()))
                ...
                .thenReturn(buildWindowData(new HashMap<>()))
                .thenReturn(buildWindowData(null))
                .thenReturn(buildWindowData(new HashMap<>()));
    ...
    @Test
    @DisplayName("doCalculate — 两个主维度，其中一个 dataMap 为 null 但另一个有 4 个 key，min 为 0")
    ...
    void testDoCalculate_SlaveSizeMismatch_NoSlaveAssigned() {
        ...
        // 两个 master 都没有 slave 设置，各自 1 个 key，min = 1
    ...
    @Test
    @DisplayName("doCalculate — velCurFlag=0 时 filterDataByMasterIndex 排除当前交易，key 减少")
    void testDoCalculate_VelCurFlagZero_ExcludesCurrentTxn() {
        ...
        Map<String, List<VelValueItem>> dataA = new HashMap<>();
        List<VelValueItem> itemsA1 = new ArrayList<>();
        itemsA1.add(new VelValueItem(TXN_ID, 2000L, BigDecimal.TEN));
        itemsA1.add(new VelValueItem("OTHER", 1000L, BigDecimal.ONE));
        dataA.put("slave_A", itemsA1);
        dataA.put("slave_extra", Arrays.asList(new VelValueItem(TXN_ID, 3000L, BigDecimal.valueOf(5))));

        Map<String, List<VelValueItem>> dataB = new HashMap<>();
        dataB.put("slave_A", new ArrayList<>(Arrays.asList(
                new VelValueItem(TXN_ID, 1000L, BigDecimal.ONE)
        )));
        ...
        // master_A: slave_A 移除 TXN_ID 后仍保留 OTHER → 1 key; slave_extra 移除 TXN_ID 后变空 → 0 key → 1 key
        // master_B: slave_A 仅 TXN_ID，全部移除 → 0 key
        // min(1, 0) = 0
        assertEquals(BigDecimal.ZERO, result.getResult());
    }
    ...
    void testDoCalculate_VelCurFlagOne_KeepsAndDoesNotDuplicate() {
        ...
        afterData.put("slave_A", new ArrayList<>(Arrays.asList(
                new VelValueItem(TXN_ID, 2000L, BigDecimal.TEN)
        )));
        afterData.put("slave_empty", new ArrayList<>());
    ...
    @Test
    @DisplayName("doCalculate — velCurFlag=1，从维度存在但列表中无当前交易，应新增当前交易到列表")
    void testDoCalculate_VelCurFlagOne_SlaveExistsWithoutCurrentTxn_AddsTxn() {
        ...
        dto.setTxnId(TXN_ID);
        dto.setTxnTime(new Date());
        dto.setVelValue(BigDecimal.valueOf(200));

        Map<String, List<VelValueItem>> afterData = new HashMap<>();
        // slave_A 存在且有 2 条其他交易，但不含当前 TXN_ID
        afterData.put("slave_A", new ArrayList<>(Arrays.asList(
                new VelValueItem("OTHER_1", 1000L, BigDecimal.ONE),
                new VelValueItem("OTHER_2", 2000L, BigDecimal.TEN)
        )));
        afterData.put("slave_B", Collections.emptyList());

        stubGetVelData(afterData);

        VelCalculateResult result = calculateMinMethod.doCalculate(dto);

        // slave_A: 2 条其他交易 + 1 条当前交易 → key 保留，key 数不变
        // slave_B: 空列表 → velCurFlag=1 → 填充当前交易 → key 保留
        // 共 2 个 key，min = 2
        assertEquals(BigDecimal.valueOf(2), result.getResult());
    }

    @Test
    @DisplayName("doCalculate — velCurFlag=1，从维度不在 dataMap 中，应新建 key 并计入")
    void testDoCalculate_VelCurFlagOne_SlaveNotInDataMap_CreatesNewKey() {
        ...
        dto.setTxnId(TXN_ID);
        dto.setTxnTime(new Date());
        dto.setVelValue(BigDecimal.valueOf(300));

        // master_A: dataMap 已有 slave_new 这个 key
        Map<String, List<VelValueItem>> dataA = new HashMap<>();
        dataA.put("slave_new", new ArrayList<>(Arrays.asList(
                new VelValueItem(TXN_ID, 1000L, BigDecimal.ONE)
        )));
        dataA.put("other_key", Collections.emptyList());

        // master_B: dataMap 中不存在 slave_new
        Map<String, List<VelValueItem>> dataB = new HashMap<>();
        dataB.put("other_key", Collections.emptyList());
        ...
        // master_A: slave_new 已有当前 TXN_ID → 不重复 → 1 key; other_key → 1 key → 共 2 key
        // master_B: slave_new 不在 dataMap → filterDataByMasterIndex 新建一个 entry → other_key(1) + 新建的 slave_new(1) → 2 key
        // min(2, 2) = 2
        assertEquals(BigDecimal.valueOf(2), result.getResult());
    }
    ...
    @Test
    @DisplayName("getCalculateMethod — 返回 MIN")
    void testGetCalculateMethod_ReturnsMin() {
        VelReadEvent dto = buildBaseDto();
