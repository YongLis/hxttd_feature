...
/**
 * VelDataSunkByCalendarMonthService 单元测试
 *
 * @author yong.li
 * @since 2026-07-04
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("VelDataSunkByCalendarMonthService 单元测试")
class VelDataSunkByCalendarMonthServiceTest {

    @InjectMocks
    private VelDataSunkByCalendarMonthService service;
    ...
    private static final String REDIS_KEY = "vel:cm:test:key";
    ...
    @Test
    @DisplayName("getTimeMode — 返回 cm")
    void testGetTimeMode() {
        assertEquals(VelocityTimeModeEnum.CURRENT_MONTH.getType(), service.getTimeMode());
    }
    ...
    void testGetVelData_CacheHit_ReturnsCacheData() {
        VelEventData dto = buildDto();
        String cacheJson = "{\"slave_A\":[{\"txnId\":\"t1\",\"ts\":3000,\"v\":50}]}";
        ...
        assertTrue(result.getBeforeData().containsKey("slave_A"));
        assertTrue(result.getAfterData().containsKey("slave_A"));
    }
    ...
    private VelEventData buildDto() {
        ...
        dto.setTxnId("TXN_002");
        dto.setFeatureCode("FEA_002");
        dto.setMasterValue("master_B");
        dto.setSlaveValue("slave_B");
        dto.setTimeMode("cm");
        dto.setTimeWindow(1);
