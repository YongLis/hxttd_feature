...
/**
 * VelDataSunkByCalendarDayService 单元测试
 *
 * @author yong.li
 * @since 2026-07-04
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("VelDataSunkByCalendarDayService 单元测试")
class VelDataSunkByCalendarDayServiceTest {

    @InjectMocks
    private VelDataSunkByCalendarDayService service;
    ...
    private static final String REDIS_KEY = "vel:cd:test:key";
    ...
    @Test
    @DisplayName("getTimeMode — 返回 cd")
    void testGetTimeMode() {
        assertEquals(VelocityTimeModeEnum.CURRENT_DAY.getType(), service.getTimeMode());
    }
    ...
    void testGetVelData_CacheHit_ReturnsCacheData() {
        ...
        assertTrue(result.getAfterData().containsKey("slave_1"));
        // beforeData 和 afterData 是同一个引用
        assertTrue(result.getBeforeData() == result.getAfterData());
    }
