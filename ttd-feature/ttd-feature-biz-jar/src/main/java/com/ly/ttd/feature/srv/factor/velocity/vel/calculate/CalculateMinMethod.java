 ...
 * 多个主维度取计算结果的最小值
 * <p>
 * 该类继承自AbstractVelCalculateMethod，用于计算多个主维度下的最小值。
 * 主要功能包括：
 * 1. 检查输入参数的有效性
 * 2. 获取并过滤数据
 * 3. 计算多个主维度的最小值
 * <p>
 * 使用场景：
 * 当需要从多个主维度中获取最小值时，可以使用该类进行计算。
 * <p>
 * 注意事项：
 * 1. 输入参数dto和masterValues不能为空，否则将返回错误。
 * 2. 支持多个主维度的计算，返回所有主维度中的最小值。
 *
 * @author yong.li
...
@Slf4j
@Service
public class CalculateMinMethod extends AbstractVelCalculateMethod {
    @Override
    protected String getCalculateMethod() {
        return VelocityCaluateTypeEnum.MIN.getCode();
    }
    ...
    public VelCalculateResult doCalculate(VelReadEvent dto) {
        ...
        if (CollectionUtils.isEmpty(dto.getMasterValues())) {
            log.warn("[VelCalculateMethod] velocity factor config error,min not support master is empty，factorCode={}", dto.getFactorCode());
            return new VelCalculateResult(BigDecimal.ZERO, "");
        }
        ...
        for (int i = 0; i < dto.getMasterValues().size(); i++) {
            ...
            if (CollectionUtils.isNotEmpty(dto.getSlaveValues())) {
                if (dto.getSlaveValues().size() == 1) {
                    ...
                    log.warn("[VelCalculateMethod] velocity factor cal min error for master slave size not equal, dto={}", JSON.toJSONString(dto));
                }
            }
        ...
        // 取最小值
        BigDecimal min = resultList.stream().min(BigDecimal::compareTo).orElse(BigDecimal.ZERO);
        return new VelCalculateResult(min, JSON.toJSONString(calculateResultMap));
    }
}
