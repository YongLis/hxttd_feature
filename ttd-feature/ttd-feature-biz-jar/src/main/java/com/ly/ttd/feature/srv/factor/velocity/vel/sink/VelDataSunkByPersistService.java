 ...
 * 永久统计服务
 * <p>
 * 该类继承自AbstractVelDataSunkService，用于永久保存维度统计数据。
 * 主要功能包括：
 * 1. 检查输入参数的有效性
 * 2. 从Redis获取缓存数据
 * 3. 更新缓存数据并返回结果
...
@Service
@Slf4j
public class VelDataSunkByPersistService extends AbstractVelDataSunkService {
    @Override
    ...
    public VelWindowData getVelData(VelEventData dto) {
        ...
        if (null == dto) {
            log.error("[VelDataSunkByPersistService] getVelData error, dto is null");
            return null;
        }
