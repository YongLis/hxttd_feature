...
import com.ly.ttd.feature.flink.task.fun.FeatureSumProcessFunction;
 ...
 * Flink 特征求和任务（动态时间窗口）
 * <p>
 * 从 Kafka 消费特征事件流，按 statedKey 分区后对 velValue 求和。
 *
 * @author yong.li
 * @since 2026/5/30
 */
public class FeatureSumTask extends AbstractTask {

    @Override
    public VelocityCaluateTypeEnum getCaluateTypeEnum() {
        return VelocityCaluateTypeEnum.SUM;
    }
    ...
    public void start() throws Exception {
        ...
        KafkaSource<VelEventData> kafkaSource = KafkaSource.<VelEventData>builder()
                .setBootstrapServers("")
                .setGroupId("feature_sum_consumer")
                .setTopics(FeatureTopic.FEATURE_SUM)
        ...
        SingleOutputStreamOperator<BigDecimal> result = streamSource
                ...
                .process(new FeatureSumProcessFunction());

        result.print();

        env.execute("FeatureSumTask");
    }
}
