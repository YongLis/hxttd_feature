...
import com.ly.ttd.feature.flink.task.fun.FeatureMinProcessFunction;
...
/**
 * Flink 特征最小值任务（动态时间窗口）
 * <p>
 * 从 Kafka 消费特征事件流，按 statedKey 分区后追踪 velValue 最小值。
 *
 * @author yong.li
 * @since 2026/5/30
 */
public class FeatureMinTask extends AbstractTask {

    @Override
    public VelocityCaluateTypeEnum getCaluateTypeEnum() {
        return VelocityCaluateTypeEnum.MIN;
    }
    ...
    public void start() throws Exception {
        ...
        KafkaSource<VelEventData> kafkaSource = KafkaSource.<VelEventData>builder()
                .setBootstrapServers("")
                .setGroupId("feature_min_consumer")
                .setTopics(FeatureTopic.FEATURE_MIN)
        ...
        SingleOutputStreamOperator<BigDecimal> result = streamSource
                ...
                .process(new FeatureMinProcessFunction());

        result.print();

        env.execute("FeatureMinTask");
    }
}
