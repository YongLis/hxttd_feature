...
(
    ...
    `offset_strategy`    VARCHAR(16)  NOT NULL DEFAULT 'earliest' COMMENT '消费偏移策略：latest/earliest/none',
    ...
    `serializer`         TEXT                  DEFAULT NULL COMMENT 'Key序列化类全限定名',
    `value_deserializer` TEXT                  DEFAULT NULL COMMENT 'Value反序列化类全限定名',
    `extra_config`       TEXT                  DEFAULT NULL COMMENT '扩展配置（K-V键值对）',
  ...
  AUTO_INCREMENT 1 COMMENT ='Kafka Topic 管理表';



CREATE TABLE `ttd_table_def`
(
    `id`          VARCHAR(64)  NOT NULL COMMENT '主键ID',
    `table_name`  VARCHAR(128) NOT NULL COMMENT '表名',
    `data_source` VARCHAR(128) NOT NULL COMMENT '库名',
    `topic`       VARCHAR(512)          DEFAULT NULL COMMENT '关联topic',
    `crt_user`    VARCHAR(128)          DEFAULT NULL COMMENT '创建人',
    `upt_user`    VARCHAR(128)          DEFAULT NULL COMMENT '修改人',
    `crt_time`    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `upt_time`    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted`     TINYINT      NOT NULL DEFAULT 0 COMMENT '逻辑删除(0-未删除,1-已删除)',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_name` (`table_name`, `data_source`),
    KEY `idx_source` (`data_source`),
    KEY `idx_name` (`table_name`)
) ENGINE = InnoDB
  AUTO_INCREMENT 1 COMMENT ='数据表定义';



CREATE TABLE `ttd_table_column`
(
    `id`          VARCHAR(64)  NOT NULL COMMENT '主键ID',
    `table_name`  VARCHAR(128) NOT NULL COMMENT '表id ',
    `column_name` VARCHAR(128) NOT NULL COMMENT '列名',
    `column_type` VARCHAR(128) NOT NULL COMMENT '类型',
    `empty` VARCHAR(8) NOT NULL COMMENT '是否允许为空：Y是 N否',
    `feature_code` VARCHAR(128) DEFAULT NULL COMMENT '指标编码',
    `crt_user`    VARCHAR(128)          DEFAULT NULL COMMENT '创建人',
    `upt_user`    VARCHAR(128)          DEFAULT NULL COMMENT '修改人',
    `crt_time`    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `upt_time`    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  ...
  AUTO_INCREMENT 1 COMMENT ='数据表列定义';

-- ========================================
-- 18. Kafka Topic 管理表 (ttd_kafka_topic)
-- 实体: KafkaTopic extends BaseEntity
-- ========================================
CREATE TABLE `ttd_kafka_topic`
(
    `id`                 VARCHAR(64)   NOT NULL COMMENT '主键ID',
    ...
    `consumer_group`     VARCHAR(128)           DEFAULT NULL COMMENT '消费者组',
    `remark`             VARCHAR(512)           DEFAULT NULL COMMENT '备注',
    `topic_status`             VARCHAR(32)   NOT NULL    DEFAULT 'INIT' COMMENT '状态：初始化INIT,审核中AUDIT，AUDIT_PASS, AUDIT_REJECT',
    `audit_reason`       VARCHAR(512)           DEFAULT NULL COMMENT '审核/驳回原因',
    `crt_user`           VARCHAR(128)           DEFAULT NULL COMMENT '创建人',
    `upt_user`           VARCHAR(128)           DEFAULT NULL COMMENT '修改人',
    `crt_time`           DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_name` (`name`)
) ENGINE = InnoDB AUTO_INCREMENT 1 COMMENT ='Kafka Topic 管理表';


CREATE TABLE `ttd_pipe_task_log`
(
    `id`          VARCHAR(64)  NOT NULL COMMENT '主键ID',
    `point_code` VARCHAR(128) NOT NULL COMMENT '接入点',
    `txn_id`  VARCHAR(128) NOT NULL COMMENT '交易号',
    `task_code` VARCHAR(200) NOT NULL COMMENT '任务编码',
    `table_name` VARCHAR(128) NOT NULL COMMENT '表名',
    `kafka_topic` VARCHAR(200) DEFAULT NULL COMMENT '绑定kafka Topic',
    `cost_time` INT DEFAULT NULL COMMENT '任务耗时，单位毫秒',
    `task_status` VARCHAR(8) DEFAULT NULL COMMENT '任务状态 succ/fail',
    ...
    KEY `idx_kafka_topic` (`kafka_topic`)
) ENGINE = InnoDB
  AUTO_INCREMENT 1 COMMENT ='数据管道任务日志';



CREATE TABLE `ttd_pipe_task`
(
    `id`          VARCHAR(64)  NOT NULL COMMENT '主键ID',
    `point_code` VARCHAR(128) NOT NULL COMMENT '接入点',
    ...
    `table_name` VARCHAR(128) NOT NULL COMMENT '表名',
    `kafka_topic` VARCHAR(200) DEFAULT NULL COMMENT '绑定kafka Topic',
    `task_status` VARCHAR(8) DEFAULT NULL COMMENT '任务状态0：开启，1未开启',
    `crt_user`    VARCHAR(128)          DEFAULT NULL COMMENT '创建人',
    `upt_user`    VARCHAR(128)          DEFAULT NULL COMMENT '修改人',
    `crt_time`    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `upt_time`    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  ...
  AUTO_INCREMENT 1 COMMENT ='数据管道任务';