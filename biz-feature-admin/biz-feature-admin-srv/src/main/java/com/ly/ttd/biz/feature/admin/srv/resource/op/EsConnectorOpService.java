...
public class EsConnectorOpService extends AbstractResourceOpService {
    ...
    public void add(BaseDto req) throws FeatureBizException {
        EsConnectorDto addReq = (EsConnectorDto) req;
        String key = projectService.getResourceKey(addReq.getProjectId(),
                FeatureResourceType.CONNECTOR_ES.getPrefix(), addReq.getResourceKey());
        ...
        ElasticsearchConnectorModel model = buildModel(addReq);
        ...
        addAudit(new AuditReq(addReq.getResourceKey(),
                ...
                null,
                JSON.toJSONString(entity), entity.getCrtUser()));
    }

    @Override
    public void update(BaseDto req) throws FeatureBizException {
        EsConnectorDto updateReq = (EsConnectorDto) req;
        ...
        ElasticsearchConnectorModel model = buildModel(updateReq);
        entity.setResourceJson(JSON.toJSONString(model));
    ...
    private ElasticsearchConnectorModel buildModel(EsConnectorDto req) {
        ElasticsearchConnectorModel model = new ElasticsearchConnectorModel();
        model.setEndpoint(req.getEndpoint());
        model.setDsl(req.getDsl());
