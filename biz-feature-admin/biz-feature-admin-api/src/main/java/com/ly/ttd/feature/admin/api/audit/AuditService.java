package com.ly.ttd.feature.admin.api.audit;

import com.ly.ttd.base.exception.BizException;
import com.ly.ttd.feature.admin.api.ServiceGroup;
import com.ly.ttd.feature.admin.api.dto.AuditDto;
import com.ly.ttd.inf.rpc.api.annotation.RpcService;

/**
 * 审核记录 RPC 服务接口
 *
 * @author yong.li
 * @since 2026-07-10
 */
@RpcService(serviceName = ServiceGroup.SERVICE_NAME)
public interface AuditService {

    /**
     * 新增审核记录
     *
     * @param dto 请求参数
     * @return 主键ID
     * @throws BizException 业务异常
     */
    Long add(AuditDto dto) throws BizException;

    /**
     * 更新审核记录
     *
     * @param dto 请求参数
     * @throws BizException 业务异常
     */
    void update(AuditDto dto) throws BizException;

    /**
     * 根据ID删除审核记录
     *
     * @param id     主键ID
     * @param opUser 操作人
     * @throws BizException 业务异常
     */
    void delete(Long id, String opUser) throws BizException;

    /**
     * 根据ID查询审核记录
     *
     * @param id 主键ID
     * @return 审核记录信息
     * @throws BizException 业务异常
     */
    AuditDto queryById(Long id) throws BizException;


    /**
     * 提交审核
     */
    void submitAudit(Long id, String approveResult, String remark, String opUser) throws BizException;
}
