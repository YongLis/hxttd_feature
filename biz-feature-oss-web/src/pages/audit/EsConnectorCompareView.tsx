...
import {DataFieldModel, EsConnectorDetailRes, getEsConnectorAuditDetail} from '@/services/srv/auditApi';
...
const EsConnectorCompareView: React.FC<Props> = ({visible, id}) => {
    const [preForm] = Form.useForm();
    const [nextForm] = Form.useForm();
    const [loading, setLoading] = useState(false);
    const [pre, setPre] = useState<EsConnectorDetailRes>();
    const [next, setNext] = useState<EsConnectorDetailRes>();

    useEffect(() => {
        setLoading(true);
        getEsConnectorAuditDetail(id)
            .then(res => {
    ...
    return (
        <Card size="small">
            ...
            {renderRow('endpoint', '索引端点', () => <Input disabled/>)}
            {renderTextAreaRow('dsl', 'DSL 查询语句', 6)}
...
export default EsConnectorCompareView;
