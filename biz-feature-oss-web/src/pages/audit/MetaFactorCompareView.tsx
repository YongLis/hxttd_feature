...
import {getMetaFactorAuditDetail, MetaFactorDetailRes} from '@/services/srv/auditApi';
...
/**
 * 基础指标对比视图 — 左右对比模式，差异行红色高亮
 */
const MetaFactorCompareView: React.FC<Props> = ({visible, id}) => {
    const [preForm] = Form.useForm();
    const [nextForm] = Form.useForm();
    const [loading, setLoading] = useState(false);
    const [pre, setPre] = useState<MetaFactorDetailRes>();
    const [next, setNext] = useState<MetaFactorDetailRes>();

    useEffect(() => {
        setLoading(true);
        getMetaFactorAuditDetail(id)
            .then(res => {
    ...
    return (
        <Card size="small">
            ...
            {renderRow('metaFieldCode', '元字段编码', () => <Input disabled/>)}
...
export default MetaFactorCompareView;
