...
import {FeatureFactorDetailRes, getFeatureFactorAuditDetail} from '@/services/srv/auditApi';
...
/**
 * 实时特征指标对比视图 — 左右对比模式，差异行红色高亮
 */
const FeatureFactorCompareView: React.FC<Props> = ({visible, id}) => {
    const [preForm] = Form.useForm();
    const [nextForm] = Form.useForm();
    const [loading, setLoading] = useState(false);
    const [pre, setPre] = useState<FeatureFactorDetailRes>();
    const [next, setNext] = useState<FeatureFactorDetailRes>();

    useEffect(() => {
        setLoading(true);
        getFeatureFactorAuditDetail(id)
            .then(res => {
    ...
    return (
        <Card size="small">
            ...
            {renderRow('featureCode', '关联特征编码', () => <Input disabled/>)}

        </Card>
    );
};

export default FeatureFactorCompareView;
