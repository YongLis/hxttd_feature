...
import {DataFieldModel, getHttpConnectorAuditDetail, HttpConnectorDetailRes} from '@/services/srv/auditApi';
...
const HttpConnectorCompareView: React.FC<Props> = ({visible, id}) => {
    const [preForm] = Form.useForm();
    const [nextForm] = Form.useForm();
    const [loading, setLoading] = useState(false);
    const [pre, setPre] = useState<HttpConnectorDetailRes>();
    const [next, setNext] = useState<HttpConnectorDetailRes>();

    useEffect(() => {
        setLoading(true);
        getHttpConnectorAuditDetail(id)
            .then(res => {
    ...
    const diffKeys = useMemo(() => {
        ...
        [
            ...
            'url', 'method', 'header', 'param', 'fields',
            'crtUser', 'crtTime',
        ].forEach(k => {
    ...
    return (
        <Card size="small">
            ...
            {renderRow('url', '请求 URL', () => <Input disabled/>)}
            {renderRow('method', '请求方法', () => <Input disabled/>)}

            {renderCustomRow('请求头', 'header',
                () => renderKeyValue(pre?.header),
                () => renderKeyValue(next?.header),
            )}

            {renderCustomRow('请求参数', 'param',
                () => renderKeyValue(pre?.param),
                () => renderKeyValue(next?.param),
            )}
...
export default HttpConnectorCompareView;
