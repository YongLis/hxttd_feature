...
interface ConnectorHttpFormValues {
    resourceKey: string;
    resourceName: string;
    version?: string;
    projectId?: number;
...
/**
 * HTTP连接器新增/编辑页面
 * - 动态变量表格 + 前置条件脚本 + 请求地址/方法/头/体配置
 */
const ConnectorHttp: React.FC = () => {
    const [form] = Form.useForm<ConnectorHttpFormValues>();
                    ...
                    if (res.code === '0000' && res.data) {
                        ...
                        if (d.resourceJson) {
                            try {
                                ...
                                setRequestBody(json.requestBody || '');
                                form.setFieldsValue({
                                    url: json.url,
                                    method: json.method,
                                    fields: json.fields || [],
            ...
            <Card>
                ...
                >
                    ...
                    <Card title="请求配置" size="small">
                        ...
                        <Form.Item name="header" label="请求头" layout='vertical'>
                            <GroovyEditor height="150px" editable={true} val={header} onChange={setHeader}/>
                        </Form.Item>
                        <Form.Item name="requestBody" label="消息" layout='vertical'>
                            <GroovyEditor height="200px" editable={true} val={requestBody} onChange={setRequestBody}/>
                        </Form.Item>
                    </Card>
