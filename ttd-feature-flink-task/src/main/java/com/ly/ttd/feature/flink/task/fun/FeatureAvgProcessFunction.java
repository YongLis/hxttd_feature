 ...
 * 平均值处理函数（动态时间窗口）
 * <p>
 * 按 statedKey 分区，对 velValue 求平均值 (sum / count)，
 * 通过 expireNum + expireType 动态计算窗口时长。
 *
 * @author yong.li
 * @since 2026/5/30
 */
public class FeatureAvgProcessFunction
    ...
    public void processElement(VelEventData event, Context ctx, Collector<BigDecimal> out)
        ...
        sumState.update(sum);
        countState.update((long) events.size());
    ...
    public void onTimer(long timestamp, OnTimerContext ctx, Collector<BigDecimal> out)
        ...
        if (sum == null || count == null || count == 0) {
            ...
            avg = sum.divide(BigDecimal.valueOf(count), 4, RoundingMode.HALF_UP);
        }
        out.collect(avg);
