 ...
 * 最小值处理函数（动态时间窗口）
 * <p>
 * 按 statedKey 分区，追踪 velValue 的最小值，
 * 通过 expireNum + expireType 动态计算窗口时长。
 *
 * @author yong.li
 * @since 2026/5/30
 */
public class FeatureMinProcessFunction
        extends KeyedProcessFunction<String, VelEventData, BigDecimal> {

    /**
     * 当前最小值
     */
    private ValueState<BigDecimal> minState;
    ...
    public void open(OpenContext ctx) throws Exception {
        minState = getRuntimeContext().getState(
                new ValueStateDescriptor<>("min-state", BigDecimal.class));
    ...
    public void onTimer(long timestamp, OnTimerContext ctx, Collector<BigDecimal> out)
            throws Exception {
        BigDecimal min = minState.value();
        out.collect(min != null ? min : BigDecimal.ZERO);
